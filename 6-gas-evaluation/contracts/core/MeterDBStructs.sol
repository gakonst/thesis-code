...
contract MeterDBStructs {
    ...
    function getCurrentMeterData(address _address) public view returns (bytes8, uint48, uint32) {
        Meter memory meter = meters[_address];
        return (
            meter.id,
            meter.power,
            meter.timestamp
    ...
    function getCurrentMeterDataById(bytes8 _id) external view returns (bytes8, uint48, uint32) {
         getCurrentMeterData(meterAddressById[_id]);
    }
    ...
    function ping(uint48 power, uint32 timestamp) external onlyMeter {
        // do all writes in memory to save gas
        Meter memory meter = meters[msg.sender]; 
        meter.lastPower = meter.power;
        meter.lastTimestamp = meter.timestamp;
        meter.power = power;
        meter.timestamp = timestamp;

        meters[msg.sender] = meter;
                
        emit Pinged(msg.sender, meter.id,  power, timestamp);
    }
