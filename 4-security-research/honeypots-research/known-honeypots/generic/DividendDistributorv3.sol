...
// This smart contract takes advantage of a Solidity compiler bug detailed here: https://etherscan.io/solcbuginfo?a=SkipEmptyStringLiteral. This bug results in skipping the empty string literal "" when passed in as a function argument. Here's the code for the function of interest: Any investor trying to divest will be in for a rude awakening when their ether is transferred to the owner's address. The bug results in this line of code this.loggedTransfer(amount, "", msg.sender, owner) being translated to this.loggedTransfer(amount, msg.sender, owner) and so the owner is the true recipient. 
