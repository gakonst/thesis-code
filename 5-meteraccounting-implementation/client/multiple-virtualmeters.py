...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description=
            """
            Launch multiple Virtual Meter instances
            """,
            usage="""
            ./multiple-virtualmeters.py -c multi-meter.json
            """)
    ...
    parser.add_argument('--config', '-c',
            help='Config file',
            default='conf/demo/simple_virtualmeters.json',
            action='store')
    ...
    try:
        ...
        cmd = './single-virtualmeter.py -k {} -f {} -a ' +args.address
        if args.debug:
            cmd += ' --debug'
        ...
        for meter_id in data:
            ...
            formula = data[meter_id]['formula']
            p = run(
                cmd.format(key, formula).split(),
                meter_id
            )
            print (meter_id, '=>', cmd.format(key, formula))
            processes.append(p)
