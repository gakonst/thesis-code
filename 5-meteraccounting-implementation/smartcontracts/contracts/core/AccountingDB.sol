...
contract AccountingDB is DSAuth {
    ...
    function activateDepartment(bytes32 departmentID) public auth {
    ...
    /// @param departmentID the ID of the department
    function deactivateDepartment(bytes32 departmentID) external auth {
    ...
    function setDelegates(bytes32 departmentID, bytes32[] delegates) external auth {
    ...
    function setVirtualMeters(bytes32 departmentID, bytes8[] virtualmeters) external auth {
    ...
    function setSubDepartments(bytes32 departmentID, bytes32[] subdepartments) external auth {
    ...
    function setHeadcount(bytes32 departmentID, uint8 headcount) external auth {
    ...
    function fixedSplit(bytes32 from, uint8 pct) external auth {
        bytes32[] memory to = accounting[from].delegates; // the receiving departments are the delegates
        require(to.length == 2);
        require(pct >=0 && pct <= 100);

        bytes32 department = departments[from];
        uint80 power_for_delegation = department.getPower();
    ...
    /// @param from the ID of the sending department
    function headcountSplit(bytes32 from) external auth {
    ...
    function billPower(bytes32 departmentID, uint80 power) external auth {
        bytes32 department = departments[departmentID];
    ...
    function clearDepartment(bytes32 departmentID, uint80 cleared) public auth { // an accountant can call that too
        bytes32 department = departments[departmentID];
